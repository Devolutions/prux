...
pub fn construct_request(request: Request<Body>, new_uri: Uri, headers: Option<HashMap<String, String>>) -> Request<Body> {
...
pub fn get_forwarded_ip(req: &Request<Body>) -> Option<IpAddr> {
    ...
    forwarded_ip.and_then(|ip_str| Ipv4Addr::from_str(&ip_str)
        .map(|ip| IpAddr::V4(ip))
        .ok()
